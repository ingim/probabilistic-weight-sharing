...
class MultiMnistDataset(torchvision.datasets.ImageFolder):

    def __init__(self, root, num_digits=3):

        super().__init__(root, transform=torchvision.transforms.Compose([
        ...
        for cls in self.class_to_idx:
            ...
            self.idx_to_class[idx] = int(cls)

    def _class_to_onehot(self, num):
        nd = self.num_digits
        label = torch.zeros(10)
        for i in range(nd):
            k = (num % 10 ** (nd - i)) // (10 ** (nd - i - 1))
            label[k] = 1
        return label

    def __getitem__(self, item):
        image, idx = super().__getitem__(item)
        target = self._class_to_onehot(self.idx_to_class[idx])
        return image, target
...
def argparser():
    ...
    parser.add_argument('--mnist_path', type=str, default='./dataset/mnist/',
                        help='path to *.gz files')
    parser.add_argument('--multimnist_path', type=str, default='./dataset/multimnist')
    ...
    parser.add_argument('--image_size', type=int, nargs='+',
                        default=[32, 96])
    parser.add_argument('--num_image_per_class', type=int, default=128)
